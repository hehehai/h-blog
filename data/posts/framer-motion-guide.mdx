---
title: 使用 Framer Motion 创建激发快乐的动画的指南
...
在过去的几个月里， [Framer Motion](https://www.framer.com/motion/introduction/) 从我玩的一个有趣的工具变成了我前端项目的核心元素，为我的 UI 添加一层交互。我从对动画和转场几乎一无所知，到能够[编排涉及大量元素的更复杂的动画](https://twitter.com/MaximeHeckel/status/1327647479521701889?s=20)。

我已经在[Twitter](https://twitter.com/MaximeHeckel)上分享了我在博客中散布的很多动画作品，你们中的很多人都要求我分享更多的代码片段。因此，我觉得是时候写点东西了！

在这篇文章中，您将找到一份简明的指南，其中包含我在 Framer Motion 方面学到的所有内容、动画的关键概念，以及如何使用此库创建动画，通过一些交互式示例和小部件激发快乐。
...
1. “我的元素在开始时在哪里/如何？” 即初始状态
2. “它需要去哪里，或者它最后需要采取哪种形状？” 即目标状态
3. “它将如何从初始状态过渡到最终状态？” 即过渡状态

在 Framer 运动的情况下，该库为我们提供了一个 `motion` 组件，该组件具有 3 个属性（道具），让我们定义上述 3 个问题的答案：
...
👉 您可以将 `animate` prop 替换为更具体的[手势 props](https://www.framer.com/api/motion/gestures/) 之一，例如 `whileHover` 或 `whileTap` 。他们可以采用我们刚刚看到的相同的“动画对象”。

👉 只需一个 `animate` 或任何一个手势道具即可定义动画 Framer Motion 组件。

👉 该库为 `initial` 未定义的智能默认值提供 `transition` 智能默认值。它甚至可以根据您在 `animate` 道具中设置的属性来调整过渡类型（弹簧、补间、缓动）！

</Callout>

现在我们了解了基础知识，让我们来看看我们的第一个例子！您将在下面找到一系列动画组件，您可以随意编辑和调整这些组件。至于要调整的内容，以下列表包含一些有趣的点，您可以查看：

- **从第一个组件中删除 ** `transition` prop （Example1）。请注意，此翻译动画从一种 `ease` 类型转到另一种 `spring` 类型。这来自我们刚才提到的“智能默认值”。
...
> 想在进入下一部分之前走得更远吗？以下是相关文档的链接： 如何在 Framer Motion 中制作动画。
...
好吧，你别担心！ Framer Motion 还允许您将变体定义为函数。每个变体作为一个函数可以接受一个参数并返回和动画对象。该参数必须在运动组件的 `custom` 道具中传递。下面的示例展示了 variant as function 的示例，无论是否单击按钮，悬停变体都将返回不同的对象。按钮 `isClicked` 的状态在运动组件的 `custom` 道具中传递。
...
- 如果单击按钮，则使按钮不缩小到其原始大小。提示：您可以使用我们上面 💡 提到的 `custom` 道具。
...
乍一看，它可能看起来很密集，但您会注意到它由我们在前面的部分和示例中单独看到的元素组成：
...
- 父子项：“父项首先出现，然后子项以 1 秒的间隔一个接一个地出现”
...
- 您可以尝试将重复类型从 `mirror` 更改为 并 `loop` 观察重复类型的细微变化。
- 使动画无限期重复，而不仅仅是 3 次。
...
Framer Motion 为我们提供了过渡对象执行此 `delayChildren` 操作的选项：
...
最重要的是，如果我们不仅要延迟孩子作为一个群体，还要根据每个孩子的兄弟姐妹延迟，例如，让他们在前一个兄弟姐妹出现后出现 1 秒，该怎么办？好吧，我们很幸运，因为有一种简单的方法可以做到这一点 `staggerChildren`.
...
哇，我们刚刚学到了很多关于 Framer Motion 的知识！我们从构建非常基本的动画（如翻译）到编排涉及多个组件的更复杂的动画，并使用 `useMotionValue` 和 `useTransform` 将多个过渡连接在一起。你现在已经了解了我所知道的关于 Framer Motion 的几乎所有知识，并且可以开始在你自己的前端工作中加入一些惊人的动画。
