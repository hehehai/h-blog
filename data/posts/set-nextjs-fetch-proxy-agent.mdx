...
🤩 [`@google/generative-ai@0.2.1`](https://github.com/google/generative-ai-js/commit/c64fca126afff14a69bdb5d6d110869ecd0e7fd6) 版本支持传递 `requestOptions` 来设置请求参数, 虽然这次更新主要为了支持 `timeout` ，但是依然可以传递其他参数。
...
在源码内，可以看到 `requestOptions` 是直接被传递进去的, 只是 `timeout` 做了单独的判断。
